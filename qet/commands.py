...
from .exceptions import QetError, PackageNotFoundError, MethodNotAvailableError, ExecutionError
...
def resolve_method(qet_name: str, method_override: str = None) -> str:
    ...
    if not package_def:
        raise PackageNotFoundError(f"Package '{qet_name}' not found in definitions database.")

    method_to_use = None
    if method_override:
        if method_override not in package_def:
            raise MethodNotAvailableError(f"Method '{method_override}' is not available for '{qet_name}'.")
    ...
    if not method_to_use:
        raise MethodNotAvailableError(f"Could not find a suitable installation method for '{qet_name}'.")
        
    return method_to_use

def add_package(qet_name: str, method_override: str = None, status_callback=None) -> Dict[str, Any]:
    ...
    if method_to_use in ["appimage", "deb", "rpm"]:
        if method_to_use == "appimage":
            dest_dir = Path(conf["defaults"].get("appimage_dir", "~/.local/bin")).expanduser()
    ...
    _log_event("install", qet_name, "success", method=method_to_use, details=context.get("destination_path") or context.get("package_name"))
    
    status("Syncing Qetfile...")
    # Auto-update global Qetfile
    qetfile_data = config.get_qetfile()
    if not any(p['qet_name'] == qet_name for p in qetfile_data.get('packages', [])):
        qetfile_data.setdefault("packages", []).append({"qet_name": qet_name, "method": method_to_use})
...
def remove_package(qet_name: str, status_callback=None) -> None:
    ...
    if not package_to_remove:
        raise QetError(f"Package '{qet_name}' is not logged as installed by qet.")

    method = package_to_remove['method']
    context = { "package_name": package_to_remove.get("details"), "destination_path": package_to_remove.get("details") }
    ...
    _log_event("remove", qet_name, "success", method=method, details=context.get("destination_path") or context.get("package_name"))
    
    status("Syncing Qetfile...")
    # Auto-update global Qetfile
    qetfile_data = config.get_qetfile()
    qetfile_data['packages'] = [p for p in qetfile_data.get('packages', []) if p['qet_name'] != qet_name]
    config.save_qetfile(qetfile_data)
...
def upgrade_package(qet_name: str = None, status_callback=None) -> List[str]:
    ...
    if failed:
        raise QetError(f"Upgrade completed with errors. Failed: {', '.join(failed)}")
...
def _log_event(action: str, qet_name: str, event_status: str, method: str = None, details: str = None):
...
def get_history(since_date: str = None, package_name: str = None) -> List[Dict[str, Any]]:
...
def search_system_managers(package_name: str) -> List[str]:
    ...
    for manager in conf.get('priority', []):
        if manager in conf.get('exclude', []): continue
...
def init_environment(status_callback=None) -> None:
    ...
    all_known_managers = ["apt", "dnf", "pacman", "zypper", "brew", "snap", "flatpak", "pip", "uvx", "cargo"]
    installed_managers = [m for m in all_known_managers if utils.is_command_available(m)]
    
    # 1. Identify Default System Manager (Must be first)
    system_primary_candidates = ["apt", "dnf", "pacman", "zypper"]
    default_manager = next((cand for cand in system_primary_candidates if cand in installed_managers), None)
    ...
    if default_manager == "apt":
        ...
        if has_snap: sandbox_managers.append("snap")
        if has_flatpak: sandbox_managers.append("flatpak")
    else:
    ...
    other_managers = [m for m in ["brew", "uvx", "pip", "cargo"] if m in installed_managers]
            
    # 4. Abstract methods (only if their underlying dependencies exist)
    abstract_methods = ["appimage", "script"]
    if default_manager == "apt": abstract_methods.append("deb")
    if default_manager == "dnf": abstract_methods.append("rpm")
    
    # Assemble Priority (No duplicates, default manager ALWAYS first)
    priority = []
    if default_manager: priority.append(default_manager)
